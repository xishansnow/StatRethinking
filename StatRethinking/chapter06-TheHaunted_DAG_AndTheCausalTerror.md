...
在腿长示例中，很容易看出在模型中包含两条腿有点傻。但在真实数据集中出现的问题是，我们可能无法预料到高度相关的预测变量之间会发生冲突。因此，我们可能会错误地解读后验分布，认为这两个预测变量都不重要。在本节中，我们将通过真实数据查看此问题的示例。

让我们回到本章前面的灵长类动物奶质数据


在本例中，我们关注 perc.fat（脂肪百分比）和 perc.lactose（乳糖百分比）变量。我们将使用这些来模拟总能量含量，kcal.per.g。上面的代码已经将这三个变量标准化了。你将使用这三个变量来探索多重共线性的自然情况。请注意，这些列中没有缺失值 NA，因此此处无需提取完整案例。但是你可以放心，quap 与像 lm 这样鲁莽的函数不同，它永远不会默默地丢弃案例。

首先将 kcal.per.g 建模为 perc.fat 和 perc.lactose 的函数，但采用两个二元回归。回顾第 5 章（第 147 页），对这些先验的讨论。


bF 和 bL 的后验分布本质上是彼此的镜像。 bF 的后验平均值与 bL 的平均值为负时一样正。两者都是狭窄的后验分布，几乎完全位于零的一侧或另一侧。考虑到每个预测因子与结果的强关联，我们可以得出结论，这两个变量都是跨物种牛奶总能量的可靠预测因子。脂肪越多，牛奶中的卡路里就越多。乳糖越多，牛奶中的卡路里越少。但是看看当我们将两个预测变量放在同一个回归模型中时会发生什么
...
现在 bF 和 bL 的后验均值更接近于零。并且这两个参数的标准偏差是双变量模型（m6.3 和 m6.4）的两倍。

这与腿长示例中的统计现象相同。发生的情况是变量 perc.fat 和 perc.lactose 包含许多相同的信息。它们几乎是彼此的替代品。因此，当你将两者都包含在回归中时，后验分布最终会描述同样合理的 bF 和 bL 组合的长脊。在脂肪和乳糖的情况下，这两个变量基本上形成一个单一的变化轴。看到这一点的最简单方法是使用对图

我在图 6.3 中显示了这个图。沿着对角线，变量被标记。在对角线外的每个散点图中，纵轴变量是同一行标记的变量，横轴变量是同一列标记的变量。例如，图 6.3 中第一行的两个散点图是 kcal.per.g（垂直）对 perc.fat（水平），然后是 kcal.per.g（垂直）对 perc.lactose（水平）。请注意，脂肪百分比与结果呈正相关，而乳糖百分比与结果呈负相关。现在查看中间行中最右侧的散点图。该图是脂肪百分比（垂直）与乳糖百分比（水平）的散点图。请注意，这些点几乎完全沿着一条直线排列。这两个变量是负相关的，而且非常强，以至于它们几乎是多余的。两者都有助于预测 kcal.per.g，但一旦你已经了解另一个，则两者都没有帮助。

在科学文献中，你可能会遇到各种应对多重共线性的狡猾方法。他们中很少有人采取因果关系的观点。有些领域实际上教学生在拟合模型之前检查成对相关性，以识别和丢弃高度相关的预测因子。这是个错误。成对相关不是问题。重要的是条件关联——而不是相关性。即便如此，正确的做法将取决于导致共线性的原因。仅凭数据中的关联不足以决定做什么。

牛奶示例中可能发生的情况是哺乳动物母亲必须遵守的牛奶成分的核心权衡。如果一个物种经常哺乳，那么牛奶往往是水汪汪的，能量低。这种牛奶含糖量高（乳糖）。相反，如果一个物种很少在短时间内哺乳，那么牛奶的能量需要更高。这种牛奶的脂肪含量非常高。这意味着一个像这样的因果模型
 
中央权衡决定了牛奶的密度 D。我们没有观察到这个变量，所以它被圈起来了。然后测定脂肪 F 和乳糖 L。最后，F 和 L 的组成决定了卡路里 K。如果我们可以测量 D，或者有一个进化和经济模型来根据物种的其他方面来预测它，那将比通过回归绊倒更好.

多重共线性问题是拟合模型问题家族的一员，该家族有时称为不可识别性。当参数不可识别时，这意味着数据和模型的结构无法估计参数的值。有时这个问题是由模型编码错误引起的，但许多重要类型的模型都存在不可识别或弱可识别的参数，即使编码完全正确。即使模型是正确的，大自然也不欠我们简单的推理。

通常，无法保证可用数据包含有关感兴趣参数的大量信息。如果这是真的，你的贝叶斯机将返回与先验非常相似的后验分布。因此，将后验与先验进行比较可能是一个好主意，这是一种查看模型从数据中提取多少信息的方法。当后验和先验相似时，并不意味着计算是错误的——你得到了你所问问题的正确答案。但它可能会让你问一个更好的问题

## 6.2 后处理偏差

担心因忽略预测变量而导致的错误推论是很常见的。这种错误通常被称为遗漏变量偏差，前一章的例子说明了这一点。担心由包含变量引起的错误推论要少得多。但包括变量偏差是真实的。小心翼翼的随机实验和不受控制的观察性研究一样容易被破坏。盲目地将变量投入因果沙拉从来都不是一个好主意。

包含变量偏差有多种形式。第一个是治疗后偏倚。 治疗后偏倚在所有类型的研究中都是一种风险。 “后处理”一词实际上来自对实验设计的思考。例如，假设你正在温室中种植一些植物。你想知道在不同抗真菌土壤处理下的生长差异，因为植物上的真菌往往会降低它们的生长。植物最初播种并发芽。他们的身高是测量出来的。然后应用不同的土壤处理。最后的措施是植物的高度和真菌的存在。这里有四个感兴趣的变量：初始高度、最终高度、处理和真菌的存在。

最终高度是感兴趣的结果。但是模型中应该包含哪些其他变量？如果你的目标是对治疗进行因果推断，则不应包括真菌，因为它是治疗后效应。

让我们模拟一些数据，使示例更加透明，看看当我们包含后处理变量时到底出了什么问题
...
在本章的开头，我认为科学研究表明可信度和新闻价值之间存在负相关关系的必要条件是选择过程——资助和期刊审查——同时关注两者。现在我想解释这种相同的选择现象如何在统计模型中发生。当它发生时，它会严重扭曲我们的推论，这种现象称为对撞机偏差。

让我们为这个例子考虑一个 DAG。该模型是可信度 (T) 和新闻价值 (N) 与提交给资助审查小组的研究提案群体无关。但它们都影响资金的选择（S）。这是图表：

两个箭头进入 S 的事实意味着它是一个碰撞器。核心概念很容易理解：当你以对撞机为条件时，它会在其原因之间创建统计（但不一定是因果关系）关联。在这种情况下，一旦你了解到某个提案已被选中 (S)，那么了解其可信度 (T) 也会提供有关其新闻价值 (N) 的信息。为什么？因为，例如，如果一个选定的提案可信度低，那么它必须具有高新闻价值。否则它不会得到资助。反过来也是一样：如果提案的新闻价值较低，我们会推断它的可信度必须高于平均水平。否则它不会被选中进行资助。

这是在选定提案的总体中产生 T 和 N 之间负相关的信息现象。这意味着我们必须注意选择我们的观察样本的过程，并且可能会扭曲变量之间的关联。但是，当你将碰撞器作为预测变量包含在内时，同样的现象也会在统计模型中产生误导性的关联。如果不小心，可能会做出错误的因果推断。让我们考虑一个扩展的例子。 

### 6.3.1 虚假悲伤的碰撞者。

考虑一下衰老如何影响幸福的问题。如果我们对人们进行了一项对他们的幸福程度进行评分的大型调查，那么年龄是否与幸福有关？如果是这样，这种关联是因果关系吗？在这里，我想向你展示如何控制看似合理的幸福混淆实际上会导致对年龄影响的推断产生偏差。

假设，只是为了挑衅，个人的平均幸福是一种在出生时就确定并且不会改变的特征随着年龄。然而，幸福确实会影响一个人的生活。这些事件之一是婚姻。更幸福的人更有可能结婚。另一个影响婚姻的变量是年龄：你活得越久，你最终结婚的可能性就越大。将这三个变量放在一起，这就是因果模型：


幸福（H）和年龄（A）都导致婚姻（M）。因此，婚姻是一个对撞机。即使幸福和年龄之间没有因果关系，如果我们以婚姻为条件——这意味着，如果我们将它作为回归的预测因素包括在内——那么它就会导致年龄和幸福之间存在统计关联。这可能会误导我们认为幸福会随着年龄而变化，而实际上它是恒定的。

为了让你相信这一点，让我们再做一次模拟。模拟在这些例子中很有用，因为只有在这些时候我们才知道真正的因果模型。如果一个程序无法在模拟示例中找出真相，我们不应该相信它在真实示例中。这次我们将进行更高级的模拟，使用基于代理的衰老和婚姻模型来生成用于回归的模拟数据集。模拟设计如下：
 (1) 每年有 20 人出生，其幸福值分布均匀。
 (2) 每一年，每个人都会变老一岁。幸福不会改变。
 (3) 年满 18 岁时，个人可以结婚。每年结婚的几率与个人的幸福感成正比。
 (4) 结婚后，个人保持婚姻状态。
 (5) 65岁以后，个人离开样本。 （他们搬到了西班牙。）
 
 我已经把这个算法写进了 rethinking 包中。你可以将其运行 1000 年并收集结果数据

 这些数据包括从出生到 65 岁各个年龄段的 1300 人。变量对应于上述 DAG 中的变量，模拟本身服从 DAG。
 
 我在图 6.4 中绘制了这些数据，将每个人显示为一个点。填充点为已婚人士。年龄在水平方向，幸福在垂直方向，最幸福的人在顶部。在 18 岁时，他们可以结婚，然后每年都会有越来越多的人结婚。因此，随着年龄的增长，更多的人结婚了。但在所有年龄段，最幸福的人都更有可能结婚。

 ![](https://gitee.com/XiShanSnow/imagebed/raw/master/images/stats-20211215210729-f56a.webp)

 > 图 6.4 模拟数据，假设幸福是均匀分布的，永远不会改变。每个点都是一个人。已婚个体用填充的蓝点显示。在 18 岁以后的每个年龄段，最幸福的人都更有可能结婚。在晚年，更多的人倾向于结婚。婚姻状况是年龄和幸福的对撞机：A → M ← H。如果我们在回归中以婚姻为条件，它会误导我们认为幸福会随着年龄的增长而下降

 假设你遇到这些数据并想询问年龄是否与幸福有关。你不知道真正的因果模型。但是你有理由认为婚姻状况可能会令人困惑。如果已婚人士平均而言或多或少幸福，那么你需要以婚姻状况为条件，以推断年龄与幸福之间的关系。
 
 因此，让我们考虑一个多元回归模型，该模型旨在推断年龄对幸福的影响，同时控制婚姻状况。这只是一个简单的多元回归，就像本章和上一章中的其他回归一样。线性模型是这样的：

 其中mid[i]是个体i婚姻状况的指标，1表示单身，2表示已婚。这只是第 4 章中的分类变量策略。当我们使用多个截距，每个类别一个截距时，比使用指示变量更容易制作先验。

 现在我们应该尽我们的职责并考虑先决条件。让我们首先考虑斜率 βA，因为我们如何缩放预测变量 A 将决定截距的含义。我们将只关注 18 岁或 18 岁以上的成人样本。想象一下，年龄和幸福感之间有很强的关系，幸福感在 18 岁时达到最大值，在 65 岁时达到最小值。如果我们重新调整年龄，使 18 到 65 的范围是一个单位，会更容易。这将做到

 现在这个新变量 A 的范围是从 0 到 1，其中 0 是 18 岁，1 是 65 岁。在这些数据中，幸福是一个任意的尺度，从 -2 到 +2。因此，我们想象中的最强关系，将幸福从最大到最小，具有 (2 -(-2))/1 = 4 的上升斜率。请记住，正态分布 95% 的质量包含在 2 个标准偏差内。因此，如果我们设置 4 之前一半的标准偏差，我们就是说我们预计 95% 的合理斜率小于最大强度。这不是一个很强的先验，但同样，它至少有助于将推理限制在现实范围内。现在是拦截。每个α是当Ai=0时μi的值。在这种情况下，这意味着 18 岁。所以我们需要让 α 覆盖所有幸福分数。 Normal(0,1) 会将 95% 的质量放在 -2 到 +2 区间内。
...
碰撞器偏差源于对常见结果的调节，如前一个示例所示。如果我们可以对图表进行排序，我们就可以避免它。但要看到潜在的对撞机并不总是那么容易，因为可能存在无法衡量的原因。未测量的原因仍然会导致对撞机偏差。所以我很遗憾地说，我们还必须考虑我们的 DAG 可能会闹鬼的可能性。
...
在本章和前一章中，有几个例子说明了我们如何使用多元回归来处理混杂。但我们也看到了多重回归如何导致混淆——控制错误的变量会破坏推理。希望我已经成功地让你远离仅仅将所有内容添加到模型中并希望回归能够解决它，并激励你相信有效的推理是可能的，如果我们足够小心和足够的知识。

但是哪些原理可以解释为什么有时省略变量有时添加变量会产生相同的现象？是否还有其他因果怪物潜伏在那里，困扰着我们的图表？我们需要一些原则来将这些示例结合在一起。

让我们将混杂定义为结果 Y 与感兴趣的预测变量 X 之间的关联与实际情况不同的任何上下文，如果我们已经通过实验确定了 X.95 的值。例如，假设我们感兴趣教育 E 与工资 W 之间的关联。问题在于，在一个典型的人群中，有许多未观察到的变量 U 会影响 E 和 W。例如一个人住在哪里，他们的父母是谁，他们的朋友是谁。这就是 DAG 的样子
...
> 图 6.6。四大元素混杂。任何有向无环图都是根据这些基本关系构建的。从左到右：X ⊥⊥ Y|Z 在 Fork 和 Pipe 中，X ̸⊥⊥ Y|Z 在碰撞器中，对后代 D 的调节就像对其父 Z 的调节

图 6.6 显示了每个元素关系的 DAG。每个 DAG，无论多么庞大和复杂，都是由这四种关系构建而成的。让我们考虑每个，从左到右。

(1) 第一种关系是我们上面使用的关系，一个叉：X ←Z →Y。这是经典的混杂因素。在分叉中，某些变量 Z 是 X 和 Y 的共同原因，从而在它们之间产生相关性。如果我们以 Z 为条件，那么 learningX 不会告诉我们关于 Y 的任何信息。X 和 Y 是独立的，以 Z 为条件。
...
现在考虑通过 B 的第二条路径。这条路径确实包含一个碰撞器，U → B ← C。因此它已经关闭。这就是为什么上面的adjustmentSets没有提到B。事实上，如果我们在B上做条件，它会打开路径，造成混淆。然后我们对 X →Y 的推断会发生变化，但如果没有 DAG，我们将不知道这种变化是在帮助我们还是误导我们。包含变量会改变 X → Y 系数的事实并不总是意味着系数现在更好。你可以只使用对撞机。
...
既然你知道了基本混杂，你就可以自己推导出任何 DAG 的条件独立性了。你可以使用在查找和关闭后门时学到的相同路径逻辑来查找条件独立性。你只需要关注一对变量，找到连接它们的所有路径，并找出是否有任何一组变量可以关闭它们。在一个大图中，这很麻烦，因为有很多对变量和可能有很多路径。但是你的电脑很擅长做这样的事。在这种情况下，存在三个隐含的条件独立性
...
多元回归不是预言机，而只是一个傀儡。这是合乎逻辑的，但它所描述的关系是有条件的关联，而不是因果影响。因此，需要来自模型外部的额外信息来理解它。本章介绍了一些常见挫折的介绍性示例：多重共线性、后处理偏差和碰撞器偏差。这些挫折的解决方案可以在一个连贯的框架下组织起来，其中分析变量之间的假设因果关系以应对混淆。在所有情况下，因果模型都存在于统计模型之外，并且难以测试。然而，在没有实验的情况下，有可能得出有效的因果推断。这是个好消息，因为出于实际和道德原因，我们经常无法进行实验
